...
class FrontStagePipeline:
    ...
    async def respond(self, msg: Msg) -> tuple[Optional[Msg], Optional[Msg]]:
        """前台并行响应。
        ...
        4. 返回 (chat_result, emotion_result) 供上层继续使用

        Args:
            msg: 用户输入消息。

        Returns:
            (chat_result, emotion_result)：两者都可能为 None（异常时）。
        """
        ...
        return chat_result, emotion_result
